...
async function fetchReviews(): Promise<Review[]> {
  const { data, error } = await supabase
    .from("reviews")
    .select("id,name,country,tour,rating,comment,created_at")
...
export function Reviews() {
  ...
  const mutation = useMutation({
    ...
    onError: () => toast.error(t.reviews.form.error),
  });
    ...
    <section id="reviews" className="border-t border-hairline bg-background">
      <div className="mx-auto w-full max-w-[1600px] px-6 md:px-10">
        ...
        <div className="grid grid-cols-1 border-x border-t border-hairline lg:grid-cols-12">
          {/* Featured review */}
          <div className="relative border-b border-hairline p-8 lg:col-span-7 lg:border-b-0 lg:border-e lg:p-14">
          ...
          {/* Rating form */}
          <div className="p-8 lg:col-span-5 lg:p-14">
            ...
            <form onSubmit={submit} className="mt-10 space-y-4">
              ...
              <OutlineLink type="submit" disabled={mutation.isPending} className="w-full">
        ...
        {reviews.length > 1 && (
          <ul className="grid grid-cols-1 border-x border-t border-hairline sm:grid-cols-2 lg:grid-cols-3">
            {reviews.slice(0, 6).map((r, i) => (
              <Reveal as="li" key={r.id} delay={i * 50} className="border-b border-hairline p-8 sm:[&:nth-child(odd)]:border-e">
