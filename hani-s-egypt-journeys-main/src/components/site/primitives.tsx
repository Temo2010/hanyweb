...
export function PlusMark({ className }: { className?: string }) {
  return (
    <span aria-hidden="true" className={cn("pointer-events-none absolute z-10 select-none text-[11px] leading-none opacity-50", className)}>
      +
    </span>
  );
}
...
export function OutlineLink({
  ...
  const base = cn(
    "label-mono group inline-flex items-center justify-center gap-3 border px-6 py-4 transition-colors duration-300",
    tone === "dark"
      ? "border-hairline text-foreground hover:border-accent hover:bg-accent hover:text-accent-foreground"
